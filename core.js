...
bot.on("message",(msg) =>   {
    ...
    bot.on("guildMemberAdd", member => {
        member.guild.channels.get('740375028415201290').send(member.mention.user.username + ':Bem vindo ao melhor serve de todos:)')
    });

        bot.on("guildMemberRemove", member => {
        member.guild.channels.get('740368716117835806').send(member.mention.user.username + ':Adeus :cold_sweat: ')
    })
